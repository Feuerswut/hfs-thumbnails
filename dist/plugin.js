...
exports.init = async api => {
    ...
    const { fileSystem } = api.require('fs');
    ...
    return {
        middleware(ctx) {
            ...
            return async () => {
                ...
                const defaultFormat = (api.getConfig('defaultFormat')).toLowerCase()
                ...
                if (qS) selectedLongSide = getNextLargest(qS)
                else if (qW && qH) {
                    const max = Math.max(qW || 0, qH || 0)
                    selectedLongSide = getNextLargest(max)
                } else if (qW) selectedLongSide = getNextLargest(qW)
                else if (qH) selectedLongSide = getNextLargest(qH)
                ...
                async function generateAndStore(fmt, sizeLong, optW, optH) {
                    try {
                        ...
                        const quality = Number(api.getConfig('quality'));
                        let outBuf;
                        let chosenFmt;
